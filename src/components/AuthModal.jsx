...
const API = "https://https-github-com-monukushawaha987.onrender.com";
...
export default function AuthModal({ open, setOpen, mode, onAuth }){
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-80">
      <form onSubmit={submit} className="bg-white rounded-xl p-6 w-96 shadow-md">
        <h3 className="text-lg font-semibold mb-3">{mode === 'signup' ? 'Create account' : 'Login'}</h3>
        {mode === 'signup' && (
          <input required placeholder="Name" value={name} onChange={e=>setName(e.target.value)} className="w-full mb-2 p-2 border rounded" />
        )}
        ...
        <div className="flex justify-end gap-2">
          ...
          <button disabled={loading} type="submit" className="px-4 py-1 rounded bg-indigo-600 text-white">{loading? 'Please wait' : (mode==='signup' ? 'Sign up' : 'Login')}</button>
        </div>
      </form>
    </div>
  )
